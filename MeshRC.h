...
namespace MeshRC
{
...
#else
	typedef std::function<void(const unsigned char *, int)> esp_rc_data_callback_t;
#endif
	...
	esp_now_send_cb_t sendHandler = [](uint8_t *addr, uint8_t sendStatus)
	{
		...
		{
#ifndef NO_ESPHOME
			ESP_LOGD("MeshRC", "message succesfully sent");
#endif
		}
		if (sendStatus == 1)
		{
#ifndef NO_ESPHOME
			ESP_LOGW("MeshRC", "message not succesfully sent");
#endif
			sent_error++;
		}
		sending = false;
